...
<% 
	...
	String sql = "SELECT date_format(c._date,_utf8'%W, %b %d') AS `day`, c.col_id, c.title, " +
					...
					"s.name,c.location,c.owner_id,u.name owner,c.video_url,s.affiliation, " +
					...
					"time_format(timediff(c.lastupdate,convert_tz(c.lastupdate,@@session.time_zone,'+00:00')),'%H%i') _timezone " +
    ...
    try{
		...
		out.println("<updated>" + today + "</updated>");
		out.println("<author>");
		out.println("<name>Chirayu Wongchokprasitti</name>");
		out.println("<email>chw20@pitt.edu</email>");
		out.println("</author>");
        ...
        while(rs.next()){
        	...
        	String _timezone = rs.getString("_timezone");
        	...
        	String atomdate = rs.getString("atomdate");

			out.println("<entry>");
			out.println("<title><![CDATA[" + title.trim() + "]]></title>");
			out.println("<updated>" + _lastupdate + " " + _timezone + "</updated>");
			...
			if(rsExt.next()){
				...
				content += "<br/><b>Tags:</b>&nbsp;<a href=\"http://halley.exp.sis.pitt.edu/comet/tag.do?tag_id=" + _tag_id + "\">" + tag + "</a>";
			}
			while(rsExt.next()){
				String tag = rsExt.getString("tag");
				long _tag_id = rsExt.getLong("tag_id");
