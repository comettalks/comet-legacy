...
<%
	...
	String sql = "SELECT c.col_id,c.title,date_format(c._date,'%b %e') talkdate, " +
	    			...
	    			"(SELECT DISTINCT u.col_id " +
	    			"FROM userprofile u,colloquium c " +
	    			"WHERE u.col_id = c.col_id AND " +
...
%>
				<div style="color:#0080ff;cursor:pointer;" 
					onmouseover="this.style.textDecoration='underline'" 
					onmouseout="this.style.textDecoration='none'"
					onclick="window.location='recentAnnotation.do<%if(affiliate_id>0)out.print("?affiliate_id="+affiliate_id);%>'">More &gt;&gt;</div>
<%	
	}else{
