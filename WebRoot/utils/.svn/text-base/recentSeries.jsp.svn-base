...
%>
				<li>
					<a href="series.do?series_id=<%=series_id %>" style="text-decoration: none;font-size: 0.75em;"
						onmouseover="this.style.textDecoration='underline'" 
						onmouseout="this.style.textDecoration='none'"
						>
					<%=rs.getString("name")%></a>
					<span style="font-size: 0.6375em;">
						<%=_talk_no.length() > 0?"&nbsp;" + _talk_no + "&nbsp;":""%>
						<%=_bookmark_no.length() > 0?"&nbsp;" + _bookmark_no + "&nbsp;":""%>
					</span>
<%-- 
					<a href="PreCreateSeries.do?series_id=<%=series_id %>"
...
<%-- 
					<span class="spansubsid<%=series_id %>" id="spansubrsid<%=series_id %>" style="display: <%=subno==0?"none":"inline" %>;font-size: 0.6375em;">
						<img border="0" src="images/subscribe.gif" /> <%=subno %> <%=subno>1?"people":"person" %>
						&nbsp;
					</span>
					<span class="spanlikesid<%=series_id %>" id="spanlikersid<%=series_id %>" style="display: <%=likeno==0?"none":"inline" %>;font-size: 0.6375em;">
...
<%-- 
						<a class="alikesid<%=series_id %>" href="javascript:return false;" 					
							...
							onmouseover="this.style.textDecoration='underline'" 
							onmouseout="this.style.textDecoration='none'"
							onclick="likeSeries(<%=ub.getUserID() %>,<%=series_id %>,this,'spanlikersid<%=series_id %>')"
						><%=liked?"Unlike":"Like" %></a>
